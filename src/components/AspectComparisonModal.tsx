              ...
              {(['1:1', '4:5', '9:16'] as AspectRatio[]).map((ratio) => {
                ...
                return (
                  <button
                    ...
                    className="relative flex cursor-pointer items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold transition-colors"
                  >
                    {isActive && (
                      <motion.span
                        layoutId="aspect-modal-pill"
                        transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                        className="absolute inset-0 rounded-xl bg-neutral-900 shadow-xs dark:bg-white"
                      />
                    )}
                    <span
                    ...
                    <span
                      className={`relative z-10 text-[10px] transition-colors ${
                        isActive ? 'text-neutral-300 dark:text-neutral-600' : 'opacity-70'
                      }`}
                    >
                      ({rMeta.description})
                    </span>
